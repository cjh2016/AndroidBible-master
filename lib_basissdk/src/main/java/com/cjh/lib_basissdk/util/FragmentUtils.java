 ...
 *     time  : 2017/1/17
 *     desc  : Fragment相关工具类, 基于@see android.support.v4.app.Fragment;
 * </pre>
 */
public final class FragmentUtils {
	...
	/************这里用一个0x01(刚好占八个bit),通过左移来表示八种不同的状态,节省内存,有点类似{@link MeasureSpec}*************/
	...
	/**
	 * 移除当前fragment和在它上面的所有fragment
	 */
	private static final int TYPE_REMOVE_TO_FRAGMENT = 0x01 << 2;
	/**
	 * 替换一个Fragment
	 */
	private static final int TYPE_REPLACE_FRAGMENT =   0x1 << 3;
	/**
	 * 弹出一个栈顶的fragment在加入另外一个fragment
	 */
	private static final int TYPE_POP_ADD_FRAGMENT =   0x1 << 4;
	/**
	 * 隐藏一个Fragment
	 */
	private static final int TYPE_HIDE_FRAGMENT =      0x1 << 5;
	/**
	 * 显示一个Fragment
	 */
	private static final int TYPE_SHOW_FRAGMENT =      0x1 << 6;
	/**
	 * 隐藏一个以前显示的Fragment
	 */
	private static final int TYPE_HIDE_SHOW_FRAGMENT = 0x1 << 7;
	...
	/**
	 * 出栈到指定fragment(在它上面的所有fragment也会出栈)
	 * @param fragmentManager
	 * @param fragmentClass
	 * @param isIncludeSelf  是否包括它自己也出栈
	 * @return 出栈成功<br>{@code false}: 出栈失败
	 */
	...
	private static Fragment operateFragment(@NonNull FragmentManager fragmentManager,
		...
		switch (type) {
			...
			case TYPE_POP_ADD_FRAGMENT:   //先弹出栈顶的fragment,在加入新的fragment
		...
		ft.commitAllowingStateLoss(); //忽略异常
		return destFragment;
	}
	...
	/**
     * 获取目标fragment的后一个fragment
     *
     * @param destFragment 目标fragment
     * @return 目标fragment的后一个fragment
     */
	public static Fragment getNextFragment(@NonNull Fragment destFragment) {
